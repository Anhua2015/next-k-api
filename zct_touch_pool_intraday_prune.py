...
逻辑已合并为每 2h 全宇宙 walk（6h 窗口 + 统一硬阈值），见 `zct_vwap_asset_pool_daily_job.py`。
...
def main() -> None:
    ...
    logger.warning(
        "zct_touch_pool_intraday_prune 已合并入每 2h 全量刷新，本入口将执行 --once 全宇宙扫描"
    )
    ap = build_parser()
    ns = ap.parse_args(["--once", "--worth-watch-plus-default-22"])
    run_once(ns)
