...
from orb.kk.vnpy.binance_gateway import (  # noqa: E402
...
class TestBinanceGatewayHelpers(unittest.TestCase):
    def test_kk_vt_symbol(self):
        self.assertEqual(kk_vt_symbol("intc"), "INTCUSDT_SWAP_BINANCE.GLOBAL")
        self.assertEqual(kk_vt_symbol("INTC"), "INTCUSDT_SWAP_BINANCE.GLOBAL")

    def test_kk_symbol_from_vt(self):
        self.assertEqual(kk_symbol_from_vt("INTCUSDT_SWAP_BINANCE.GLOBAL"), "INTCUSDT")
        self.assertEqual(kk_symbol_from_vt("INTCUSDT_SWAP_BINANCE"), "INTCUSDT")
...
class TestKkBinanceGatewayGuards(unittest.TestCase):
    ...
    def test_shadow_rejects_order(self, mock_super):
        with mock.patch.dict(os.environ, {"KK_SHADOW": "1"}, clear=False):
            gw = self._gateway()
            out = gw.send_order(self._open_req())
        self.assertEqual(out, "")
        mock_super.assert_not_called()

    @mock.patch("orb.kk.vnpy.binance_gateway.live_enabled", return_value=False)
    @mock.patch("orb.kk.vnpy.binance_gateway.BinanceLinearGateway.send_order")
    ...
    @mock.patch("orb.kk.vnpy.binance_gateway.BinanceLinearGateway.send_order", return_value="BINANCE_LINEAR.1")
    ...
    def test_max_open_positions_rejects(self, mock_super, mock_cfg, _live):
        mock_cfg.return_value = KKConfig(
            live_enabled=True,
    ...
    @mock.patch("orb.kk.vnpy.binance_gateway.record_vnpy_fill")
    @mock.patch("orb.kk.vnpy.binance_gateway.live_enabled", return_value=True)
    @mock.patch("orb.kk.vnpy.binance_gateway.BinanceLinearGateway.on_trade")
