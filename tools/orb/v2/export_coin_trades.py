#!/usr/bin/env python3
"""导出 COIN 推荐方案（OR10 3% tw0）交易明细 CSV。"""
...
from tools.orb.v2.sim_live_session import TRADE_CSV_FIELDS, _write_trades_csv, simulate_live_sessions  # noqa: E402
...
def main() -> int:
    ...
    detail = _enrich_trades(trades, df5, cfg, or_minutes=10)

    # 原始 sim 字段 + 钱包列
    raw_path = OUT_DIR / "coin_or10_3pct_tw0.trades.csv"
    pd.DataFrame(trades).to_csv(raw_path, index=False, encoding="utf-8-sig")

    detail_path = OUT_DIR / "coin_or10_3pct_tw0_detail.csv"
    detail.to_csv(detail_path, index=False, encoding="utf-8-sig")

    legacy = ROOT / "output" / "orb_coin_trades_detail.csv"
    detail.to_csv(legacy, index=False, encoding="utf-8-sig")

    wins = int(detail["win"].sum())
    losses = int((~detail["win"]).sum())
    print(
        f"COIN OR10 3% tw0 | {len(dates)} sessions | {len(trades)} trades | "
        f"win={wins} loss={losses} | net={bt['wallet_net_usdt']:+.2f}U"
    )
