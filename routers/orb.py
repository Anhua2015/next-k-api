...
from orb.core.db import clear_orb_tables, migrate_orb_tables, sync_symbol_bots_pool
...
def load_summary() -> Dict[str, Any]:
    ...
    try:
        ...
        sync_symbol_bots_pool(cur, symbols, initial_equity_usdt=robot_equity)
        conn.commit()
        cur.execute(
...
def load_signals(*, limit: int, offset: int, symbol: Optional[str], status: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if universe and not symbol:
            rows = [r for r in rows if str(r.get("symbol") or "").upper() in universe]
        for d in rows:
            d["status"] = _status(d)
            rid = d.get("robot_id")
